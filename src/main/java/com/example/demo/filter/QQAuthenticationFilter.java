...
public class QQAuthenticationFilter extends AbstractAuthenticationProcessingFilter {
	...
	@Override public Authentication attemptAuthentication(HttpServletRequest httpServletRequest,
		...
		System.out.println(JSON.toJSON(qqToken));
		if (qqToken != null) {
	...
	private QQToken getToken(String tokenAccessApi) throws IOException {
		...
		if(results.length == 3) {
			...
			int expiresInt = Integer.valueOf(results[1].replace("expires_in=", ""));
			String refreshToken = results[2].replace("refresh_token", "");
			qqToken.setAccessToken(accessToken);
			qqToken.setExpiresIn(expiresInt);
